...
> 基于 **Mihomo** 内核的代理中转网关控制面，将远程机场订阅收敛至服务器端，统一向局域网 / 公网设备暴露稳定的 SOCKS5 / HTTP 代理入口。
...
- **一键部署** — 提供 systemd 模板与自动化安装脚本，支持 Ubuntu 开箱即用
...
控制面 (`proxyrelayd`) 负责配置、状态、订阅和 UI；数据面 (`mihomo`) 负责协议承载和流量转发。
...
项目自带 `service.sh` 脚本，可将 ProxyRelay 注册为系统服务并管理其生命周期。
